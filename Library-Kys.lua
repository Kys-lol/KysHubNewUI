...
if not isfolder("Gluto") then
    makefolder("Gluto")
end
if not isfolder("Gluto/Config") then
    makefolder("Gluto/Config")
end

local placeId = game.PlaceId
local nameCacheFile = "Gluto/GameName_" .. tostring(placeId) .. ".txt"
local gameName = ""
...
ConfigFile = "Gluto/Config/" .. gameName .. ".json"
ConfigFolder = "Gluto/Configs"
...
function LoadConfigElements()
    ...
    for key, element in pairs(Elements) do
        if ConfigData[key] ~= nil and element.Set then
            ...
            if not ok then
                warn("[ZiaanUi] Gagal menerapkan config untuk elemen '" .. tostring(key) .. "': " .. tostring(err))
            end
        end
    end
    ApplyingConfig = false
...
do
    ...
    if ok and result then
        ...
        warn("[ZiaanUi] Gagal memuat library resmi Lucide Icons dari " .. lucide_source_url .. " -- " .. tostring(result))
        Lucide = nil
    end
end
...
local function ApplyIcon(imageLabel, name, size)
    ...
    if asset then
        ...
        warn("[ZiaanUi] Ikon Lucide tidak ditemukan atau library gagal dimuat: \"" .. name .. "\"")
        imageLabel.Image = ""
        imageLabel.ImageRectOffset = Vector2.new(0, 0)
        imageLabel.ImageRectSize = Vector2.new(0, 0)
    end
...
local WebImageCacheFolder = "Gluto/ImageCache"
...
local function GetWebImageAsset(url)
    ...
    if not (writefile and getAsset and game.HttpGet) then
        warn("[ZiaanUi] Executor tidak mendukung getcustomasset/getsynasset, gambar dari url web mungkin tidak tampil.")
        return ""
    end
    ...
    if result == "" then
        ...
        if ok and res and res ~= "" then
            ...
            warn("[ZiaanUi] Gagal memuat gambar dari url: " .. tostring(url) .. " -- " .. tostring(res))
        end
    end
...
function Chloex:MakeNotify(NotifyConfig)
    ...
    NotifyConfig.Title = NotifyConfig.Title or "Gluto"
...
--         FileName   = "Gluto/Config/key.txt",  -- (opsional) kalau dikosongkan/tidak diisi,
--                                                    -- otomatis jadi "Gluto/Config/<Folder>_key.txt"
...
local function RunKeySystemGate(KeyConfig, AccentColor, ScopeName)
    ...
    KeyConfig.Title    = KeyConfig.Title or "Gluto"
    ...
    if not KeyConfig.FileName or KeyConfig.FileName == "" then
        ...
        if scope ~= "" then
            KeyConfig.FileName = "Gluto/Config/" .. scope .. "_key.txt"
        else
            KeyConfig.FileName = "Gluto/Config/key.txt"
        end
    end
    ...
    local function SaveValidKey(key)
        ...
        if not ok then
            warn("[ZiaanUi] Gagal menyimpan key ke " .. tostring(KeyConfig.FileName))
        end
    end
...
function Chloex:Window(GuiConfig)
    ...
    GuiConfig.Title        = GuiConfig.Title or "Gluto"
    ...
    ConfigFile        = "Gluto/Config/" .. FolderScope .. ".json"
    ...
    function GuiFunc:ExportConfig()
        ...
        if setclipboard then
            ...
            than("Config copied to clipboard", 4, GuiConfig.Color, "Gluto", "Export")
        end
        return payload
    end
    ...
    function GuiFunc:ImportConfig(str)
        if not str or str == "" then
            than("Paste a config string first", 4, Color3.fromRGB(255, 90, 90), "Gluto", "Import")
            return false
        end
        local ok, dec = pcall(function() return HttpService:JSONDecode(str) end)
        if not ok or type(dec) ~= "table" then
            than("Invalid config format", 4, Color3.fromRGB(255, 90, 90), "Gluto", "Import")
            return false
        end
        local data = ExtractConfigPayload(dec)
        if not data then
            than("Config data is empty", 4, Color3.fromRGB(255, 90, 90), "Gluto", "Import")
            return false
        end

        ApplyConfigData(data)
        QueueSaveConfig(AutoSaveEnabled)
        than("Config imported", 4, GuiConfig.Color, "Gluto", "Import")
        return true
    end
    ...
    function GuiFunc:SaveConfigAs(name)
        if not name or name == "" then
            than("Enter a config name first", 4, Color3.fromRGB(255, 90, 90), "Gluto", "Config")
            return false
        end
        ...
        if not ok then
            ...
            than("Failed to save config", 4, Color3.fromRGB(255, 90, 90), "Gluto", "Config")
            return false
        end

        writefile(path, encoded)
        SetActiveConfig(name, path, false, "saved")
        than("Saved '" .. name .. "'", 4, GuiConfig.Color, "Gluto", "Config")
        return true
    end
    ...
    function GuiFunc:LoadConfigByName(name)
        ...
        if not (isfile and isfile(path)) then
            than("Config '" .. tostring(name) .. "' not found", 4, Color3.fromRGB(255, 90, 90), "Gluto", "Config")
            return false
        end
        local ok, dec = pcall(function() return HttpService:JSONDecode(readfile(path)) end)
        if not ok or type(dec) ~= "table" then
            than("Failed to read config", 4, Color3.fromRGB(255, 90, 90), "Gluto", "Config")
            return false
        end
        local data = ExtractConfigPayload(dec)
        if not data then
            than("Config data is empty", 4, Color3.fromRGB(255, 90, 90), "Gluto", "Config")
            return false
        end

        ApplyConfigData(data)
        SetActiveConfig(name, path, false, "manual")
        than("Loaded '" .. name .. "'", 4, GuiConfig.Color, "Gluto", "Config")
        return true
    end
    ...
    function GuiFunc:DeleteConfig(name)
        ...
        if isfile and isfile(path) and delfile then
            ...
            than("Deleted '" .. name .. "'", 4, Color3.fromRGB(255, 170, 0), "Gluto", "Config")
            return true
        end
        return false
    end
    ...
    if GuiConfig.Discord and GuiConfig.Discord ~= "" then
        ...
        DiscordBtn.Activated:Connect(function()
            if setclipboard then
                ...
                than("Discord invite copied", 4, GuiConfig.Color, "Gluto", "Community")
            end
        end)
    ...
    function Tabs:AddTab(TabConfig)
        ...
        local function AttachCustomGui(parent, guiObject, config)
            ...
            if typeof(guiObject) ~= "Instance" or not guiObject:IsA("GuiObject") then
                warn("[ZiaanUi] AddCustomGUI membutuhkan Roblox GuiObject (Frame, TextLabel, TextButton, ImageLabel, ScrollingFrame, dll.).")
                return nil
            end
        ...
        function Sections:AddSection(Title, AlwaysOpen)
            ...
            function Items:AddConfig(ConfigCfg)
                ...
                AutoSaveToggle = Items:AddToggle({
                    ...
                    Callback = function(value)
                        ...
                        if value then
                            than("Auto Save diaktifkan", 4, GuiConfig.Color, "Gluto", "Config")
                        else
                            than("Auto Save dimatikan", 4, Color3.fromRGB(255, 170, 0), "Gluto", "Config")
                        end
                    end,
                })
                ...
                AutoToggle = Items:AddToggle({
                    ...
                    Callback = function(value)
                        ...
                        if value and selectedConfig then
                            ...
                            than("Auto load set to '" .. selectedConfig .. "'", 4, GuiConfig.Color, "Gluto", "Config")
                        elseif value then
                            GuiFunc:SetAutoLoad("")
                            if AutoToggle then AutoToggle:Set(false, true) end
                            than("Select a config first", 4, Color3.fromRGB(255, 170, 0), "Gluto", "Config")
                        else
                            GuiFunc:SetAutoLoad("")
                        end
                    end,
                        ...
                        Click.MouseButton1Click:Connect(function()
                            ...
                            if CardConfig.Link then
                                if setclipboard then
                                    ...
                                    than("Link copied to clipboard", 4, GuiConfig.Color, "Gluto", "Widget")
                                end
                            end
                        end)
                    end
            ...
            function Items:AddPresetManager(PresetConfig)
                ...
                local presetFilePath = "Gluto/Config/Presets_" .. gameName .. ".json"
                ...
                CreateBtn("Save", Color3.fromRGB(36, 36, 36), function()
                    if currentPresetName == "" or not currentPresetName then
                        than("Masukkan nama preset terlebih dahulu!", 3, Color3.fromRGB(255, 170, 0), "Gluto", "Preset")
                        return
                    end
                    ...
                    than("Preset '" .. currentPresetName .. "' disimpan!", 3, GuiConfig.Color, "Gluto", "Preset")
                end)
                ...
                CreateBtn("Delete", Color3.fromRGB(60, 25, 25), function()
                    ...
                    if PresetFunc.Presets[currentPresetName] then
                        ...
                        NameBox.Text = ""
                        RefreshDropdown()
                        than("Preset berhasil dihapus.", 3, Color3.fromRGB(255, 170, 0), "Gluto", "Preset")
                    else
                        than("Preset tidak ditemukan.", 3, Color3.fromRGB(255, 90, 90), "Gluto", "Preset")
                    end
                end)
