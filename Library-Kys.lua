...
if not isfolder("Kys") then
    makefolder("Kys")
end
if not isfolder("Kys/Config") then
    makefolder("Kys/Config")
end

local placeId = game.PlaceId
local nameCacheFile = "Kys/GameName_" .. tostring(placeId) .. ".txt"
local gameName = ""
...
ConfigFile = "Kys/Config/" .. gameName .. ".json"
ConfigFolder = "Kys/Configs"
...
function LoadConfigElements()
    ...
    for key, element in pairs(Elements) do
        if ConfigData[key] ~= nil and element.Set then
            ...
            if not ok then
                warn("[KysUi] Gagal menerapkan config untuk elemen '" .. tostring(key) .. "': " .. tostring(err))
            end
        end
    end
    ApplyingConfig = false
...
-- Footagesus Icons V2 backend
-- Supports Lucide, Craft, Geist, Solar, SF Symbols, Gravity UI, and custom icon types
local icons_source_url = "https://raw.githubusercontent.com/Footagesus/Icons/main/Main-v2.lua"

local IconsV2
do
    local ok, result = pcall(function()
        return loadstring(game:HttpGetAsync(icons_source_url))()
    end)
    if ok and result then
        ...
        warn("[KysUi] Gagal memuat Footagesus Icons V2 dari " .. icons_source_url .. " -- " .. tostring(result))
        IconsV2 = nil
    end
end
...
local function ResolveIconType(iconName)
    if type(iconName) ~= "string" then
        return nil, iconName
    end

    -- Explicit namespace support, e.g. solar:Home2Bold / sfsymbols:HouseFill
    local prefix, rawName = iconName:match("^([%w_%-]+):(.+)$")
    if prefix and rawName then
        local aliases = {
            lucide = "lucide",
            craft = "craft",
            geist = "geist",
        ...
        local iconType = aliases[string.lower(prefix)]
        if iconType then
            return iconType, rawName
        end
    end
...
local function GetIconAsset(iconName, size)
    if not IconsV2 or type(iconName) ~= "string" then return nil end

    local iconType, rawName = ResolveIconType(iconName)
    local cacheKey = iconName .. ":" .. tostring(size or 256)
    local cached = IconAssetCache[cacheKey]
    if cached ~= nil then
    ...
    local ok, asset = pcall(function()
        -- Always restore the default type for unprefixed icons because
        -- SetIconsType() changes the module's global current icon set.
        IconsV2.SetIconsType(iconType or "lucide")
        return IconsV2.GetIcon(rawName)
    end)

    if ok and asset then
        IconAssetCache[cacheKey] = asset
        return asset
    end

    IconAssetCache[cacheKey] = false
    return nil
end
...
local function ApplyIcon(imageLabel, name, size)
    ...
    local asset = GetIconAsset(name, size)
    if asset then
        -- Footagesus Icons V2 GetIcon() returns:
        -- { imageId, { ImageRectSize = ..., ImageRectPosition = ..., Parts = ... } }
        -- It may also return a raw rbxassetid string for custom/direct icons.
        if type(asset) == "table" and asset[1] then
            imageLabel.Image = tostring(asset[1])
            local meta = asset[2]
            if type(meta) == "table" then
                imageLabel.ImageRectOffset = meta.ImageRectPosition or Vector2.new(0, 0)
                imageLabel.ImageRectSize = meta.ImageRectSize or Vector2.new(0, 0)
            else
                imageLabel.ImageRectOffset = Vector2.new(0, 0)
                imageLabel.ImageRectSize = Vector2.new(0, 0)
            end
        else
            imageLabel.Image = tostring(asset)
            imageLabel.ImageRectOffset = Vector2.new(0, 0)
            imageLabel.ImageRectSize = Vector2.new(0, 0)
        end
    else
        warn("[KysUi] Ikon tidak ditemukan atau Footagesus Icons V2 gagal dimuat: \"" .. name .. "\"")
        imageLabel.Image = ""
        imageLabel.ImageRectOffset = Vector2.new(0, 0)
        imageLabel.ImageRectSize = Vector2.new(0, 0)
    end
...
local WebImageCacheFolder = "Kys/ImageCache"
...
local function GetWebImageAsset(url)
    ...
    if not (writefile and getAsset and game.HttpGet) then
        warn("[KysUi] Executor tidak mendukung getcustomasset/getsynasset, gambar dari url web mungkin tidak tampil.")
        return ""
    end
    ...
    if result == "" then
        ...
        if ok and res and res ~= "" then
            ...
            warn("[KysUi] Gagal memuat gambar dari url: " .. tostring(url) .. " -- " .. tostring(res))
        end
    end
...
function CircleClick(Button, X, Y)
    task.spawn(function()
        ...
        Circle.ImageColor3 = Color3.fromRGB(66, 66, 66)
...
function Chloex:MakeNotify(NotifyConfig)
    ...
    NotifyConfig.Title = NotifyConfig.Title or "Kys"
    ...
    task.spawn(function()
        ...
        if not CoreGui.NotifyGui:FindFirstChild("NotifyLayout") then
            ...
            NotifyLayout.BorderColor3 = Color3.fromRGB(26, 26, 26)
        ...
        NotifyFrame.BackgroundColor3 = Color3.fromRGB(26, 26, 26)
        NotifyFrame.BorderColor3 = Color3.fromRGB(26, 26, 26)
        ...
        NotifyFrameReal.BackgroundColor3 = Color3.fromRGB(26, 26, 26)
        NotifyFrameReal.BorderColor3 = Color3.fromRGB(26, 26, 26)
        ...
        Top.BorderColor3 = Color3.fromRGB(26, 26, 26)
        ...
        TextLabel.BorderColor3 = Color3.fromRGB(26, 26, 26)
        ...
        TextLabel1.BorderColor3 = Color3.fromRGB(26, 26, 26)
        ...
        Close.Text = ""
        Close.TextColor3 = Color3.fromRGB(26, 26, 26)
        Close.TextSize = 14
        Close.AnchorPoint = Vector2.new(1, 0.5)
        Close.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
        Close.BackgroundTransparency = 0.9990000128746033
        Close.BorderColor3 = Color3.fromRGB(26, 26, 26)
        ...
        ImageLabel.BorderColor3 = Color3.fromRGB(26, 26, 26)
        ...
        TextLabel2.BorderColor3 = Color3.fromRGB(26, 26, 26)
...
--         FileName   = "Kys/Config/key.txt",  -- (opsional) kalau dikosongkan/tidak diisi,
--                                                    -- otomatis jadi "Kys/Config/<Folder>_key.txt"
...
local function RunKeySystemGate(KeyConfig, AccentColor, ScopeName)
    ...
    KeyConfig.Title    = KeyConfig.Title or "Kys"
    ...
    if not KeyConfig.FileName or KeyConfig.FileName == "" then
        ...
        if scope ~= "" then
            KeyConfig.FileName = "Kys/Config/" .. scope .. "_key.txt"
        else
            KeyConfig.FileName = "Kys/Config/key.txt"
        end
    end
    AccentColor        = KeyConfig.Color or AccentColor or Color3.fromRGB(150, 150, 150)
    ...
    local function SaveValidKey(key)
        ...
        if not ok then
            warn("[KysUi] Gagal menyimpan key ke " .. tostring(KeyConfig.FileName))
        end
    end
    ...
    Shadow.ImageColor3 = Color3.fromRGB(23, 23, 23)
    ...
    KeyMain.BackgroundColor3 = Color3.fromRGB(18, 18, 18)
    ...
    InputStroke.Color = Color3.fromRGB(49, 49, 49)
    ...
    GetKeyBtn.BackgroundColor3 = Color3.fromRGB(36, 36, 36)
    ...
    SubmitBtn.TextColor3 = Color3.fromRGB(26, 26, 26)
    ...
    GetKeyBtn.MouseEnter:Connect(function()
        TweenService:Create(GetKeyBtn, TweenInfo.new(0.15), { BackgroundColor3 = Color3.fromRGB(44, 44, 44) }):Play()
    end)
    GetKeyBtn.MouseLeave:Connect(function()
        TweenService:Create(GetKeyBtn, TweenInfo.new(0.15), { BackgroundColor3 = Color3.fromRGB(36, 36, 36) }):Play()
    end)
...
function Chloex:Window(GuiConfig)
    ...
    GuiConfig.Title        = GuiConfig.Title or "Kys"
    ...
    ConfigFile        = "Kys/Config/" .. FolderScope .. ".json"
    ...
    function GuiFunc:ExportConfig()
        ...
        if setclipboard then
            ...
            than("Config copied to clipboard", 4, GuiConfig.Color, "Kys", "Export")
        end
        return payload
    end
    ...
    function GuiFunc:ImportConfig(str)
        if not str or str == "" then
            than("Paste a config string first", 4, Color3.fromRGB(255, 90, 90), "Kys", "Import")
            return false
        end
        local ok, dec = pcall(function() return HttpService:JSONDecode(str) end)
        if not ok or type(dec) ~= "table" then
            than("Invalid config format", 4, Color3.fromRGB(255, 90, 90), "Kys", "Import")
            return false
        end
        local data = ExtractConfigPayload(dec)
        if not data then
            than("Config data is empty", 4, Color3.fromRGB(255, 90, 90), "Kys", "Import")
            return false
        end

        ApplyConfigData(data)
        QueueSaveConfig(AutoSaveEnabled)
        than("Config imported", 4, GuiConfig.Color, "Kys", "Import")
        return true
    end

    local function EnsureConfigFolder()
        if not isfolder("Kys") then makefolder("Kys") end
    ...
    function GuiFunc:SaveConfigAs(name)
        if not name or name == "" then
            than("Enter a config name first", 4, Color3.fromRGB(255, 90, 90), "Kys", "Config")
            return false
        end
        ...
        if not ok then
            ...
            than("Failed to save config", 4, Color3.fromRGB(255, 90, 90), "Kys", "Config")
            return false
        end

        writefile(path, encoded)
        SetActiveConfig(name, path, false, "saved")
        than("Saved '" .. name .. "'", 4, GuiConfig.Color, "Kys", "Config")
        return true
    end
    ...
    function GuiFunc:LoadConfigByName(name)
        ...
        if not (isfile and isfile(path)) then
            than("Config '" .. tostring(name) .. "' not found", 4, Color3.fromRGB(255, 90, 90), "Kys", "Config")
            return false
        end
        local ok, dec = pcall(function() return HttpService:JSONDecode(readfile(path)) end)
        if not ok or type(dec) ~= "table" then
            than("Failed to read config", 4, Color3.fromRGB(255, 90, 90), "Kys", "Config")
            return false
        end
        local data = ExtractConfigPayload(dec)
        if not data then
            than("Config data is empty", 4, Color3.fromRGB(255, 90, 90), "Kys", "Config")
            return false
        end

        ApplyConfigData(data)
        SetActiveConfig(name, path, false, "manual")
        than("Loaded '" .. name .. "'", 4, GuiConfig.Color, "Kys", "Config")
        return true
    end
    ...
    function GuiFunc:DeleteConfig(name)
        ...
        if isfile and isfile(path) and delfile then
            ...
            than("Deleted '" .. name .. "'", 4, Color3.fromRGB(255, 170, 0), "Kys", "Config")
            return true
        end
        return false
    end
    ...
    DropShadow.ImageColor3 = Color3.fromRGB(23, 23, 23)
    ...
    if GuiConfig.Discord and GuiConfig.Discord ~= "" then
        ...
        DiscordBtn.Activated:Connect(function()
            if setclipboard then
                ...
                than("Discord invite copied", 4, GuiConfig.Color, "Kys", "Community")
            end
        end)
    ...
    FooterLabel.BorderColor3 = Color3.fromRGB(26, 26, 26)
    ...
    ImageLabel1.BorderColor3 = Color3.fromRGB(26, 26, 26)
    ...
    Min.Text = ""
    Min.TextColor3 = Color3.fromRGB(26, 26, 26)
    Min.TextSize = 14
    Min.AnchorPoint = Vector2.new(1, 0.5)
    Min.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
    Min.BackgroundTransparency = 0.9990000128746033
    Min.BorderColor3 = Color3.fromRGB(26, 26, 26)
    ...
    ImageLabel2.BorderColor3 = Color3.fromRGB(26, 26, 26)
    ...
    LayersTab.BorderColor3 = Color3.fromRGB(26, 26, 26)
    ...
    DecideFrame.BorderColor3 = Color3.fromRGB(26, 26, 26)
    ...
    Layers.BorderColor3 = Color3.fromRGB(26, 26, 26)
    ...
    NameTab.BorderColor3 = Color3.fromRGB(26, 26, 26)
    ...
    LayersReal.BorderColor3 = Color3.fromRGB(26, 26, 26)
    ...
    ScrollTab.ScrollBarImageColor3 = Color3.fromRGB(26, 26, 26)
    ScrollTab.ScrollBarThickness = 0
    ScrollTab.Active = true
    ScrollTab.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
    ScrollTab.BackgroundTransparency = 0.9990000128746033
    ScrollTab.BorderColor3 = Color3.fromRGB(26, 26, 26)
    ...
    if GuiConfig.ShowInfo then
        ...
        InfoAvatar.BackgroundColor3 = Color3.fromRGB(36, 36, 36)
    ...
    if GuiConfig.Search then
        ...
        SearchResults.BackgroundColor3 = Color3.fromRGB(23, 23, 23)
    ...
    Close.Activated:Connect(function()
        ...
        Overlay.Size = UDim2.new(1, 0, 1, 0)
        Overlay.BackgroundColor3 = Color3.fromRGB(26, 26, 26)
        ...
        Dialog.ImageColor3 = Color3.fromRGB(23, 23, 23)
        ...
        Yes.BackgroundColor3 = Color3.fromRGB(41, 41, 41)
        ...
        Cancel.BackgroundColor3 = Color3.fromRGB(41, 41, 41)
    ...
    MoreBlur.BackgroundColor3 = Color3.fromRGB(26, 26, 26)
    MoreBlur.BackgroundTransparency = 0.999
    MoreBlur.BorderColor3 = Color3.fromRGB(26, 26, 26)
    ...
    DropShadow1.ImageColor3 = Color3.fromRGB(26, 26, 26)
    ...
    DropdownSelect.BackgroundColor3 = Color3.fromRGB(35, 35, 35)
    DropdownSelect.BackgroundTransparency = 0.15
    DropdownSelect.BorderColor3 = Color3.fromRGB(26, 26, 26)
    ...
    DropdownSelectReal.BackgroundColor3 = Color3.fromRGB(26, 26, 26)
    DropdownSelectReal.BackgroundTransparency = 0.4
    DropdownSelectReal.BorderColor3 = Color3.fromRGB(26, 26, 26)
    ...
    function Tabs:AddTab(TabConfig)
        ...
        ScrolLayers.BorderColor3 = Color3.fromRGB(26, 26, 26)
        ...
        TabButton.BorderColor3 = Color3.fromRGB(26, 26, 26)
        ...
        FeatureImg.BorderColor3 = Color3.fromRGB(26, 26, 26)
        ...
        if CountTab == 0 then
            ...
            ChooseFrame.BorderColor3 = Color3.fromRGB(26, 26, 26)
        ...
        -- ============================================================
        -- Custom GUI support
        -- Allows any Roblox GuiObject (Frame, TextButton, ImageLabel,
        -- ScrollingFrame, etc.) to be embedded directly inside a tab.
        -- The library keeps it inside the tab's scrolling area and
        -- automatically updates CanvasSize through the existing layout.
        -- ============================================================
        local CustomGuiCount = 0

        local function AttachCustomGui(parent, guiObject, config)
            config = config or {}

            if typeof(guiObject) ~= "Instance" or not guiObject:IsA("GuiObject") then
                warn("[KysUi] AddCustomGUI membutuhkan Roblox GuiObject (Frame, TextLabel, TextButton, ImageLabel, ScrollingFrame, dll.).")
                return nil
            end

            CustomGuiCount = CustomGuiCount + 1

            local oldParent = guiObject.Parent
            local oldPosition = guiObject.Position
            local oldSize = guiObject.Size

            -- UIListLayout controls the root position inside the tab.
            guiObject.LayoutOrder = config.LayoutOrder or (100000 + CustomGuiCount)
            guiObject.Position = UDim2.new(0, 0, 0, 0)

            if config.Name and tostring(config.Name) ~= "" then
                guiObject.Name = tostring(config.Name)
            elseif guiObject.Name == "" then
                guiObject.Name = "CustomGUI_" .. tostring(CustomGuiCount)
            end

            if config.Size then
                guiObject.Size = config.Size
            elseif config.Width or config.Height then
                local width = config.Width or oldSize.X
                local height = config.Height or oldSize.Y

                local widthScale = typeof(width) == "UDim2" and width.X.Scale or 0
                local widthOffset = typeof(width) == "UDim2" and width.X.Offset or tonumber(width) or 0
                local heightScale = typeof(height) == "UDim2" and height.Y.Scale or 0
                local heightOffset = typeof(height) == "UDim2" and height.Y.Offset or tonumber(height) or 0

                guiObject.Size = UDim2.new(widthScale, widthOffset, heightScale, heightOffset)
            elseif oldSize.X.Scale == 0 and oldSize.X.Offset == 0 then
                -- Avoid a zero-width custom object when no usable size was set.
                guiObject.Size = UDim2.new(1, 0, oldSize.Y.Scale, oldSize.Y.Offset)
            end
            ...
            function api:SetVisible(visible)
                if guiObject and guiObject.Parent then
                    guiObject.Visible = visible ~= false
                end
                return api
            end

            function api:SetLayoutOrder(order)
                if guiObject and guiObject.Parent then
                    guiObject.LayoutOrder = tonumber(order) or guiObject.LayoutOrder
                end
                return api
            end
        ...
        -- Direct tab-level API:
        -- local Tab = Window:CreateTab(...)
        -- Tab:AddCustomGUI(MyFrame)
        function Sections:AddCustomGUI(guiObject, config)
            return AttachCustomGui(ScrolLayers, guiObject, config)
        end

        -- Alias with common casing.
        Sections.AddCustomGui = Sections.AddCustomGUI
        function Sections:AddSection(Title, AlwaysOpen)
            ...
            Section.BorderColor3 = Color3.fromRGB(26, 26, 26)
            ...
            SectionReal.BorderColor3 = Color3.fromRGB(26, 26, 26)
            ...
            SectionButton.Text = ""
            SectionButton.TextColor3 = Color3.fromRGB(26, 26, 26)
            SectionButton.TextSize = 14
            SectionButton.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
            SectionButton.BackgroundTransparency = 0.9990000128746033
            SectionButton.BorderColor3 = Color3.fromRGB(26, 26, 26)
            ...
            FeatureFrame.BackgroundColor3 = Color3.fromRGB(26, 26, 26)
            FeatureFrame.BackgroundTransparency = 0.9990000128746033
            FeatureFrame.BorderColor3 = Color3.fromRGB(26, 26, 26)
            ...
            SectionTitle.BorderColor3 = Color3.fromRGB(26, 26, 26)
            ...
            SectionDecideFrame.BorderColor3 = Color3.fromRGB(26, 26, 26)
            ...
            SectionAdd.BorderColor3 = Color3.fromRGB(26, 26, 26)
            ...
            -- Embed a custom GuiObject inside this section.
            -- Example: Items:AddCustomGUI(MyFrame, { Size = UDim2.new(1, 0, 0, 180) })
            function Items:AddCustomGUI(guiObject, config)
                local api = AttachCustomGui(SectionAdd, guiObject, config)

                -- SectionAdd's existing ChildAdded/ChildRemoved handlers will
                -- recalculate the section height. The layout listener above
                -- also updates the tab CanvasSize automatically.
                return api
            end

            Items.AddCustomGui = Items.AddCustomGUI
            ...
            function Items:AddSlider(SliderConfig)
                ...
                SliderTitle.BorderColor3 = Color3.fromRGB(26, 26, 26)
                ...
                SliderContent.BorderColor3 = Color3.fromRGB(26, 26, 26)
                ...
                SliderInput.BorderColor3 = Color3.fromRGB(26, 26, 26)
                ...
                SliderFrame.BorderColor3 = Color3.fromRGB(26, 26, 26)
                ...
                SliderDraggable.BorderColor3 = Color3.fromRGB(26, 26, 26)
                ...
                SliderCircle.BorderColor3 = Color3.fromRGB(26, 26, 26)
            ...
            function Items:AddColorPicker(ColorConfig)
                ...
                ColorConfig.Colors = ColorConfig.Colors or {
                    ...
                    Color3.fromRGB(36, 36, 36), Color3.fromRGB(0, 0, 0),
                ...
                local function OpenPalette()
                    ...
                    Dialog.BackgroundColor3 = Color3.fromRGB(26, 26, 26)
                    Dialog.BorderSizePixel = 0
                    Dialog.ZIndex = 61
                    Dialog.Parent = Overlay
                    ...
                    CancelButton.BackgroundColor3 = Color3.fromRGB(46, 46, 46)
                    ...
                    CancelButton.MouseEnter:Connect(function()
                        TweenService:Create(CancelButton, TweenInfo.new(0.15), { BackgroundColor3 = Color3.fromRGB(55, 55, 55) }):Play()
                    end)
                    CancelButton.MouseLeave:Connect(function()
                        TweenService:Create(CancelButton, TweenInfo.new(0.15), { BackgroundColor3 = Color3.fromRGB(46, 46, 46) }):Play()
                    end)
            ...
            function Items:AddInput(InputConfig)
                ...
                Input.BorderColor3 = Color3.fromRGB(26, 26, 26)
                ...
                InputTitle.BorderColor3 = Color3.fromRGB(26, 26, 26)
                ...
                InputContent.BorderColor3 = Color3.fromRGB(26, 26, 26)
                ...
                InputFrame.BorderColor3 = Color3.fromRGB(26, 26, 26)
                ...
                InputTextBox.BorderColor3 = Color3.fromRGB(26, 26, 26)
            ...
            function Items:AddKeybind(KeybindConfig)
                ...
                KeyBtn.BackgroundColor3 = Color3.fromRGB(41, 41, 41)
                ...
                local defaultBg = Color3.fromRGB(41, 41, 41)
                local defaultBgTrans = 0.2
                local defaultStroke = Color3.fromRGB(60, 60, 60)
                local defaultText = Color3.fromRGB(200, 200, 200)
                local hoverBg = Color3.fromRGB(49, 49, 49)
                local hoverStroke = Color3.fromRGB(74, 74, 74)
            ...
            function Items:AddConfig(ConfigCfg)
                ...
                AutoSaveToggle = Items:AddToggle({
                    ...
                    Callback = function(value)
                        ...
                        if value then
                            than("Auto Save diaktifkan", 4, GuiConfig.Color, "Kys", "Config")
                        else
                            than("Auto Save dimatikan", 4, Color3.fromRGB(255, 170, 0), "Kys", "Config")
                        end
                    end,
                })
                ...
                AutoToggle = Items:AddToggle({
                    ...
                    Callback = function(value)
                        ...
                        if value and selectedConfig then
                            ...
                            than("Auto load set to '" .. selectedConfig .. "'", 4, GuiConfig.Color, "Kys", "Config")
                        elseif value then
                            GuiFunc:SetAutoLoad("")
                            if AutoToggle then AutoToggle:Set(false, true) end
                            than("Select a config first", 4, Color3.fromRGB(255, 170, 0), "Kys", "Config")
                        else
                            GuiFunc:SetAutoLoad("")
                        end
                    end,
            ...
            function Items:AddBanner(BannerConfig)
                ...
                Banner.BackgroundColor3 = Color3.fromRGB(32, 32, 32)
                ...
                if BannerConfig.Version then
                    ...
                    VerPill.BackgroundColor3 = Color3.fromRGB(26, 26, 26)
            ...
            function Items:AddCardWidget(CardConfig)
                ...
                Widget.BackgroundColor3 = Color3.fromRGB(32, 32, 32)
                        ...
                        Click.MouseButton1Click:Connect(function()
                            ...
                            if CardConfig.Link then
                                if setclipboard then
                                    ...
                                    than("Link copied to clipboard", 4, GuiConfig.Color, "Kys", "Widget")
                                end
                            end
                        end)
                    end
            ...
            function Items:AddPresetManager(PresetConfig)
                ...
                local presetFilePath = "Kys/Config/Presets_" .. gameName .. ".json"
                ...
                NameBoxFrame.BackgroundColor3 = Color3.fromRGB(30, 30, 30)
                ...
                CreateBtn("Save", Color3.fromRGB(36, 36, 36), function()
                    if currentPresetName == "" or not currentPresetName then
                        than("Masukkan nama preset terlebih dahulu!", 3, Color3.fromRGB(255, 170, 0), "Kys", "Preset")
                        return
                    end
                    ...
                    than("Preset '" .. currentPresetName .. "' disimpan!", 3, GuiConfig.Color, "Kys", "Preset")
                end)
                ...
                CreateBtn("Delete", Color3.fromRGB(60, 25, 25), function()
                    ...
                    if PresetFunc.Presets[currentPresetName] then
                        ...
                        NameBox.Text = ""
                        RefreshDropdown()
                        than("Preset berhasil dihapus.", 3, Color3.fromRGB(255, 170, 0), "Kys", "Preset")
                    else
                        than("Preset tidak ditemukan.", 3, Color3.fromRGB(255, 90, 90), "Kys", "Preset")
                    end
                end)
